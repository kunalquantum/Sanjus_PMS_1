  ...
  <>
    <div className="px-5 pt-5">
      <div className="rounded-[28px] border border-white/10 bg-white/5 p-5 text-white backdrop-blur">
        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-blue-100/80">NGO Education PMS</p>
        <h1 className="mt-4 font-display text-2xl font-bold tracking-[-0.04em]">Scholarship command center</h1>
...
export const AppShell = ({ children }) => {
  ...
  return (
    <div className="h-screen bg-app-grid bg-grid text-slate-900 overflow-hidden">
          ...
          <header className="sticky top-0 z-30 border-b border-white/60 bg-white/76 backdrop-blur-xl">
            <div className="flex flex-col gap-4 px-4 py-4 lg:px-8">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-1 items-center gap-3 rounded-[24px] border border-white/80 bg-white/86 px-4 py-3 shadow-soft">
                ...
                <button
                  ...
                  className="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-soft transition hover:bg-slate-50"
                >
                  Search
                </button>
                ...
                <div className="relative">
                  <button 
                    ...
                    className="relative inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-soft transition hover:bg-slate-50 active:scale-95"
                  >
                  ...
                  {notifOpen && (
                    <>
                      ...
                      <div className="absolute right-0 top-14 z-50 w-80 overflow-hidden rounded-[28px] border border-white/60 bg-white/95 shadow-2xl backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-200">
                          ...
                          {notifications.length > 0 ? (
                            notifications.map((n) => (
                              <button
                                ...
                                className={`flex w-full flex-col gap-1 border-b border-slate-50 p-5 text-left transition hover:bg-slate-50 ${!n.read ? 'bg-brand-50/30' : ''}`}
                              >
                                <div className="flex items-center justify-between gap-3">
                                  <p className={`truncate text-sm font-bold ${!n.read ? 'text-slate-950' : 'text-slate-600'}`}>{n.title}</p>
                                  {!n.read && <div className="h-2 w-2 rounded-full bg-brand-500" />}
                                </div>
                                <p className="line-clamp-2 text-xs leading-relaxed text-slate-500">{n.description}</p>
                                <p className="mt-1 text-[10px] font-medium text-slate-400">{n.time || 'Recently'}</p>
                              </button>
                            ))
                          ) : (
                            <div className="p-10 text-center">
                              <Bell className="mx-auto h-8 w-8 text-slate-200" />
                              <p className="mt-3 text-sm text-slate-400 font-medium">All caught up!</p>
                            </div>
                          )}
                        </div>
                        <button 
                          onClick={() => { setNotifOpen(false); navigate('/notifications'); }}
                          className="w-full bg-slate-50 py-4 text-xs font-bold uppercase tracking-widest text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
                        >
                ...
                <div className="hidden items-center gap-3 rounded-[22px] border border-white/80 bg-white/90 px-4 py-2.5 shadow-soft md:flex">
                  <div className="text-right">
                    <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Prototype Mode</p>
                    <p className="text-sm font-semibold text-slate-900">{roleTone[role]}</p>
                  </div>
              ...
              {location.pathname !== '/' && (
                <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-slate-400">{breadcrumbs.join(' / ')}</p>
                    <h2 className="mt-1 font-display text-3xl font-bold tracking-[-0.04em] text-slate-950">{breadcrumbs[breadcrumbs.length - 1]}</h2>
                  </div>
                  <div className="rounded-full border border-brand-100 bg-brand-50 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-brand-700 shadow-soft">
                    {role.replace('_', ' ')}
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          <aside className="relative flex w-[320px] flex-col bg-sidebar-glow shadow-panel">
            <div className="flex items-center justify-between px-5 pt-5">
              <p className="text-sm font-bold uppercase tracking-[0.22em] text-blue-100/80">NGO PMS</p>
